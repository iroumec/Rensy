...
public:
    FrameBuffer(unsigned width, unsigned height) : width{width}, height{height}, colourBuffer(width, height, TGAImage::RGB), depthBuffer(width, height, TGAImage::GRAYSCALE) {}
    void setColour(unsigned x, unsigned y, const Colour &colour) { this->colourBuffer.set(x, y, colour); }
    void setDepth(unsigned x, unsigned y, unsigned char depth) { this->depthBuffer.set(x, y, TGAColour{{depth}}); }
    bool isCurrentDepthLower(unsigned x, unsigned y, unsigned char newDepth) const { return newDepth <= this->depthBuffer.get(x, y)[0]; }
